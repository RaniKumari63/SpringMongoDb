...
{
	when(mongoTemplate.findOne(Mockito.any(Query.class),Mockito.eq(Employee.class))).thenReturn(null);
	when(mongoTemplate.insert(employee1)).thenReturn(employee1);
	assertEquals(HttpStatus.CREATED,employeeServiceImpl.createEmployee(request).getStatusCode());
}
}
